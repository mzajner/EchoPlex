...
#include "src/Echoplex_Faust.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct Echoplex_FaustMetaData {
		SN_NODE_ID("Echoplex_Faust");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Echoplex_Faust = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Echoplex_Faust, Echoplex_FaustMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
