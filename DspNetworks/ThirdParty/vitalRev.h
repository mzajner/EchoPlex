...
#include "src/vitalRev.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct vitalRevMetaData {
		SN_NODE_ID("vitalRev");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using vitalRev = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _vitalRev, vitalRevMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
